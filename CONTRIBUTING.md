...
5. Run `python scripts/generate_baseline_report.py --output docs/baseline_results.md --check`.
6. Run `python scripts/generate_release_manifest.py --output-json docs/release_manifest.json --output-md docs/release_manifest.md --check`.
7. Confirm each case has a clear defensive purpose.
8. Confirm recommended actions are proportionate and reviewable by a human analyst.
9. Confirm labels do not depend on facts outside the alert packet.
